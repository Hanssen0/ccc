...
`Address.fromString` accepts one client. When you need to handle both networks
in the same code path, try the clients explicitly and handle the failed attempt
at the call site:
